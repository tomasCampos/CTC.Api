...
namespace CTC.Application.Features.Client.UseCases.UpdateClient.Validators
{
    internal sealed class UpdateClientRequestValidator : IRequestValidator<UpdateClientInput>
    {
        public Task<RequestValidationModel> Validate(UpdateClientInput request)
        {
            var errors = new List<string>();

            if (string.IsNullOrWhiteSpace(request.Id))
                errors.Add("O Id do cliente deve ser informado");
            if (string.IsNullOrWhiteSpace(request.Name))
                errors.Add("O nome do cliente deve ser informado.");
            if (string.IsNullOrWhiteSpace(request.Email))
                errors.Add("O E-mail do cliente deve ser informado.");
